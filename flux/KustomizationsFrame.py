...
class KustomizationsFrame(FluxCRsFrame):

  def __init__(self, frame, style):
    self.columns_keys = ("namespace", "kustomization", "message", "status", "suspended")
  ...
  def initTreeview(self):
    ...
    self.table.heading("kustomization", text="Kustomization", command=lambda: self.sortColumn(column_id="kustomization"))
  ...
  def initContextMenu(self):
    ...
    self.ctx_menu.add_command(label="Reconcile", command=lambda: threading.Thread(self.fluxCommandPopup(resource="kustomization", verb="reconcile")).start())
    self.ctx_menu.add_command(label="Reconcile with Source", command=lambda: threading.Thread(self.fluxCommandPopup(resource="kustomization", verb="reconcile", options="--with-source")).start())
    self.ctx_menu.add_command(label="Suspend", command=lambda: self.fluxCommand(resource="kustomization", verb="suspend", options="--all"))
    self.ctx_menu.add_command(label="Resume", command=lambda: self.fluxCommand(resource="kustomization", verb="resume", options="--all --wait=false"))
    self.ctx_menu.add_command(label="Suspend + Resume", command=self.suspendResume)

  def loadData(self):
    self.fluxcrs = self.k8s.getAllKustomizations()
    self.table_data = []
    try:
  ...
  def suspendResume(self):
    self.fluxCommand(resource="kustomization", verb="suspend", options="--all")
    self.fluxCommand(resource="kustomization", verb="resume", options="--all --wait=false")